...
func testCheckOceanAKSNPVirtualNodeGroupExists(launchSpec *gcp.LaunchSpec, resourceName string) resource.TestCheckFunc {
	return func(s *terraform.State) error {
...
func TestAccSpotinstOceanAKSNPVirtualNodeGroup_Baseline(t *testing.T) {
	oceanID := "o-f27b341c"
	resourceName := createOceanAKSNPVirtualNodeGroupResource(oceanID)
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestCheckResourceAttr(resourceName, "metadata.0.value", "true"),
					resource.TestCheckResourceAttr(resourceName, "metadata.1.key", "gci-update-strategy"),
					resource.TestCheckResourceAttr(resourceName, "metadata.1.value", "update_disabled"),
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestCheckResourceAttr(resourceName, "labels.1.key", "testKey2"),
					resource.TestCheckResourceAttr(resourceName, "labels.1.value", "testVal2"),
				),
			},
			{
