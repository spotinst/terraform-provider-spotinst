		...
		Steps: []resource.TestStep{
			...
			},
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					testCheckOceanAWSAttributes(&cluster, clusterName),
					resource.TestCheckResourceAttr(resourceName, "scheduled_task.#", "1"),
					resource.TestCheckResourceAttr(resourceName, "scheduled_task.4105912767.shutdown_hours.0.is_enabled", "true"),
					resource.TestCheckResourceAttr(resourceName, "scheduled_task.4105912767.shutdown_hours.0.time_windows.#", "2"),
					resource.TestCheckResourceAttr(resourceName, "scheduled_task.4105912767.shutdown_hours.0.time_windows.0", "Fri:15:30-Sat:13:30"),
					resource.TestCheckResourceAttr(resourceName, "scheduled_task.4105912767.shutdown_hours.0.time_windows.1", "Sun:00:30-Sun:01:00"),
					resource.TestCheckResourceAttr(resourceName, "scheduled_task.4105912767.tasks.#", "1"),
					resource.TestCheckResourceAttr(resourceName, "scheduled_task.4105912767.tasks.0.cron_expression", "testcron"),
					resource.TestCheckResourceAttr(resourceName, "scheduled_task.4105912767.tasks.0.is_enabled", "true"),
					resource.TestCheckResourceAttr(resourceName, "scheduled_task.4105912767.tasks.0.task_type", "clusterRoll"),
				),
			},
			{
				Config: createOceanAWSTerraform(&ClusterConfigMetadata{
...
const testSchedulingConfig_Update = `
 // --- Scheduling --------------------
  scheduled_task   {
    shutdown_hours  {
      is_enabled = true
      time_windows = ["Fri:15:30-Sat:13:30","Sun:00:30-Sun:01:00"]
    }
    tasks  {
      is_enabled = true
