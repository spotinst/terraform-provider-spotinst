...
func Setup(fieldsMap map[commons.FieldName]*commons.GenericField) {
	fieldsMap[Headrooms] = commons.NewGenericField(
		...
		func(resourceObject interface{}, resourceData *schema.ResourceData, meta interface{}) error {
			...
			var result []*azure_np.VirtualNodeGroupHeadrooms = nil
			if value, ok := resourceData.GetOkExists(string(Headrooms)); ok {
				if headrooms, err := expandHeadrooms(value); err != nil {
					return err
				} else {
...
func expandHeadrooms(headroom interface{}) ([]*azure_np.VirtualNodeGroupHeadrooms, error) {
	list := headroom.(*schema.Set).List()
	headrooms := make([]*azure_np.VirtualNodeGroupHeadrooms, 0, len(list))
	...
	for _, v := range list {
		...
		headrooms = append(headrooms, &azure_np.VirtualNodeGroupHeadrooms{
...
func flattenHeadrooms(autoScaleHeadrooms []*azure_np.VirtualNodeGroupHeadrooms) []interface{} {
