...
func Setup(fieldsMap map[commons.FieldName]*commons.GenericField) {
	fieldsMap[Filters] = commons.NewGenericField(
		...
		func(resourceObject interface{}, resourceData *schema.ResourceData, meta interface{}) error {
			...
			if cluster != nil && cluster.VirtualNodeGroupTemplate != nil && cluster.VirtualNodeGroupTemplate.VmSizes != nil &&
				cluster.VirtualNodeGroupTemplate.VmSizes.Filters != nil {
				result = flattenFilters(cluster.VirtualNodeGroupTemplate.VmSizes.Filters)
			}
			if len(result) > 0 {
		...
		func(resourceObject interface{}, resourceData *schema.ResourceData, meta interface{}) error {
			...
			if v, ok := resourceData.GetOk(string(Filters)); ok {
				if filters, err := expandFilters(v, false); err != nil {
					...
					cluster.VirtualNodeGroupTemplate.VmSizes.SetFilters(filters)
				}
			}
			return nil
		},
		...
		func(resourceObject interface{}, resourceData *schema.ResourceData, meta interface{}) error {
			...
			cluster.VirtualNodeGroupTemplate.VmSizes.SetFilters(value)
			return nil
		},
		nil,
	)
